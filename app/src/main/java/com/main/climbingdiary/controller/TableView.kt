...
class TableView (val context: Context, val view:View){
    ...
    fun createTableView() {
        //Variables
        val styles: Array<String> = Styles.getStyle(true)
        ...
        tbrow0.addView(tv0)
        for (x in 0..styles.size - 1) {
            val tv_style = TextView(context)
            tv_style.text = styles[x]
            tv_style.setPadding(20, 10, 20, 10)
            tv_style.setTextAppearance(view.context, R.style.TableHeader)
